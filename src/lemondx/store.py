"""Persistent user state: module defaults, remembered parameters, and profiles.
...
# -- profiles --------------------------------------------------------------
#
# A profile is a named module selection with its parameters. One file each, so
# a profile can be copied between machines or dropped in from a repo, and a
# file hand-edited into nonsense costs that one profile rather than all of
# them. The profile's real name is inside the file; the filename is only a slug
# of it, since names may contain spaces and mixed case.
...
def profile_slug(name):
    """A filename stem for a profile name: never empty, never a path."""
    slug = _SLUG_DASHES.sub("-", _SLUG_STRIP.sub("-", (name or "").strip().lower()))
    return slug.strip("-._") or "profile"


def _clean_profile(stored, name):
    """Normalise one record. These files are meant to be edited by hand."""
    modules = stored.get("modules")
    params = stored.get("params")
    return {
        "name": name,
        "description": str(stored.get("description") or "")[:200],
        "modules": list(dict.fromkeys(
            str(m) for m in (modules if isinstance(modules, list) else []))),
...
def _scan():
    """(path, name, record) for every readable profile file."""
    ...
    for filename in filenames:
        ...
        found.append((path, name.strip(), _clean_profile(stored, name.strip())))
    return found


def load_profiles():
    """Every profile on disk, keyed by name.

    Two files can claim the same name if one was copied in by hand. The first
    by filename wins, which is also the one save and delete act on.
    """
    profiles = {}
    for _, name, record in _scan():
        profiles.setdefault(name, record)
    return profiles


def _path_for(name):
    for path, existing, _ in _scan():
        if existing == name:
            return path
    return None
...
def _free_path(slug):
    """An unused profiles/<slug>.json, suffixed when two names slug alike."""
    directory = profiles_dir()
...
def save_profile(name, record):
    """Write one profile, reusing its file when that name already exists."""
    name = (name or "").strip()
    path = _path_for(name) or _free_path(profile_slug(name))
    payload = _clean_profile(record, name)
    with _lock:
...
def delete_profile(name):
    """Remove a profile. Returns False if there was no such name."""
    path = _path_for((name or "").strip())
    if path is None:
        return False
    try:
        os.unlink(path)
...
def prune_module(module_id):
    """Forget a module that no longer exists, in settings and in every profile.
