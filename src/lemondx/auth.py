...
class AuthService:
    ...
    def create_token(self, principal, name, role=None, expires_days=None, owner=None):
        ...
        if expires_days not in (None, "", 0):
            ...
            # float() happily accepts "1e309" and "nan"; both get past days <= 0 and
            # then raise OverflowError/ValueError inside int(), past the AuthError
            # path, so bad input would answer 500 instead of 400.
            if not math.isfinite(days) or days <= 0:
                raise AuthError("expires_days must be a positive number.", 400)
